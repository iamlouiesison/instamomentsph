...
import { EventUpgradeSchema, SUBSCRIPTION_TIERS, VIDEO_ADDON_PRICING, calculateEventPrice } from '@/lib/validations/event'
import { z } from 'zod'
...
export async function POST(
  ...
  try {
    ...
    const { data: existingEvent, error: fetchError } = await supabase
      .from('events')
      .select('id, subscription_tier, has_video_addon, status, total_photos, total_videos')
      .eq('id', eventId)
      .eq('host_id', user.id)
      .single()
    ...
    if (existingEvent.status === 'expired') {
      return NextResponse.json(
        { success: false, error: { code: 'EVENT_EXPIRED', message: 'Cannot upgrade expired events' } },
        { status: 400 }
      )
    }

    const currentTier = existingEvent.subscription_tier as keyof typeof SUBSCRIPTION_TIERS
    ...
    if (newTierIndex <= currentTierIndex) {
      return NextResponse.json(
        { success: false, error: { code: 'NOT_AN_UPGRADE', message: 'New tier must be higher than current tier' } },
        { status: 400 }
      )
    }
    ...
    const currentPrice = calculateEventPrice(currentTier, existingEvent.has_video_addon)
    const newPrice = calculateEventPrice(newTier, validatedData.hasVideoAddon)
    const upgradeCost = newPrice - currentPrice

    if (upgradeCost <= 0) {
      return NextResponse.json(
        { success: false, error: { code: 'NO_UPGRADE_COST', message: 'No additional cost for this upgrade' } },
        { status: 400 }
      )
    }
    ...
    if (paymentError) {
      ...
      return NextResponse.json(
        { success: false, error: { code: 'DATABASE_ERROR', message: 'Failed to create payment record' } },
        { status: 500 }
      )
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { success: false, error: { code: 'DATABASE_ERROR', message: 'Failed to upgrade event' } },
        { status: 500 }
      )
    }
