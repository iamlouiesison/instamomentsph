...
import { EventCreateSchema, generateGallerySlug } from '@/lib/validations/event'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const gallerySlug = generateGallerySlug(validatedData.name, validatedData.eventDate)
    ...
    if (existingEvent) {
      ...
      const eventData = {
        ...
        event_date: validatedData.eventDate ? new Date(validatedData.eventDate).toISOString().split('T')[0] : null,
