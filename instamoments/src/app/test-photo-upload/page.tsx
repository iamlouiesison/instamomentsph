...
export default function TestPhotoUploadPage() {
  const [uploadResults, setUploadResults] = useState<ProcessedImage[]>([]);
  const [uploadErrors, setUploadErrors] = useState<string[]>([]);
  const [testEventId] = useState('test-event-123'); // Mock event ID for testing

  const handleUploadComplete = (uploadedPhotos: ProcessedImage[]) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                <p><strong>Event ID:</strong> {testEventId}</p>
                <p><strong>Event Name:</strong> Test Event</p>
                <p><strong>Max Photos per User:</strong> 10</p>
              </div>
            </div>
            <div>
                  ...
                  {uploadResults.map((photo, index) => (
                    <div key={index} className="border rounded-lg p-4 space-y-2">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        <p><strong>Size:</strong> {photo.compressedSize} bytes</p>
                        <p><strong>Compression:</strong> {photo.compressionRatio}%</p>
                        <p><strong>Type:</strong> {photo.file.type}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground ml-4">
              <li>Click "Take Photo" button to test camera integration</li>
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground ml-4">
              <li>Click "Choose Files" to select multiple images</li>
            ...
            <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground ml-4">
              <li>Try uploading without entering a name (should show error)</li>
              <li>Enter a very long caption (should be limited to 200 chars)</li>
